...
double_t OPSModel::getVolume()
{
  if (_updateVolume)
  {
    ...
    for (const auto &f : _triangles)
      _volume +=
          0.166666667 * (_positions.col(f[0]).dot(
                            _positions.col(f[2]).cross(_positions.col(f[1]))));
    _updateVolume = false;
  }
  return _volume;
}
  ...
  if (_updateArea)
  {
    ...
    for (const auto &f : _triangles)
      ...
      _area += 0.5 * (_positions.col(f[1]) - _positions.col(f[0]))
                         .cross(_positions.col(f[2]) - _positions.col(f[0]))
                         .norm();
    _updateArea = false;
  }
  return _area;
...
double_t OPSModel::operator()(const VectorXd &x, VectorXd &g)
{
  ...
  _viscoEn = 0.5 * _viscosity *
             ((x.segment(0, 3 * _N) - _pX).dot(x.segment(0, 3 * _N) - _pX));
  f += (0.5 * _K_i * areaDiff - _Lambda_i) * areaDiff + _brownEn + _viscoEn;
  g.head(3 * _N) +=
      -1.0 * _brownCoeff * _xi + _viscosity * (x.segment(0, 3 * _N) - _pX);
