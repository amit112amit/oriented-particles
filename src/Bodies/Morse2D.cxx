...
namespace OPS{
    // Constructor
    Morse2D::Morse2D(size_t N, double_t &f, RefM2Xd pos, RefM2Xd grad,
	    double_t Lx, double_t Ly, double_t s):_N(N),_f(f),
    _positions(pos.data(),2,N),_posGradient(grad.data(),2,N), _searchRadius(s){
	...
	for (auto i = 0; i < _N; ++i) {
	    for(auto j: _neighbors[i]){
		_neighbors[j].erase(std::remove(_neighbors[j].begin(),
			    _neighbors[j].end(),i), _neighbors[j].end());
	    }
	}
    }
	...
	for(auto i=0; i < _N; ++i){
	    pts->InsertNextPoint( _positions(0,i),
		    _positions(1,i), 0);
	}
    ...
    void Morse2D::updateDataForKabsch(){
	_prevX = _positions;
    }
	...
	for(auto i=0; i < _N; ++i){
		...
		xj(k) += _boundaryCrossCounter[_initialNearestNeighbor[i]][k]*
		    _bounds[k];
	    }
