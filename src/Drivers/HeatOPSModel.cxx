...
int main(int argc, char* argv[]){
    ...
    std::stringstream taskIdStream, sstm;

    double_t alpha=1.0, beta=1.0, gamma=1.0, re=1.0, f = 0, percentStrain = 15,
	     s=(100/(re*percentStrain))*log(2.0);

    size_t viterMax, nameSuffix=0, step=0, N=10, saveFreq = 100000, taskId;
    ...
    // Get the task id from environment variable
    taskIdStream << std::getenv("SGE_TASK_ID");
    std::cout<< "SGE_TASK_ID = " << taskIdStream.str() << std::endl;
    taskId = std::stoul(taskIdStream.str());

    // Read the file containing mapping of task id and file suffix
    ifstream taskIdMap("TaskIdMap.txt");
    std::vector<size_t> fileSuffix;
    if(taskIdMap.good()){

	size_t temp;
	while(taskIdMap >> temp){
	    fileSuffix.push_back(temp);
	}
	taskIdMap.close();

	if(fileSuffix.size() >= taskId){
	    taskId = fileSuffix[taskId - 1];
	    std::cout<< "File suffix = " << taskId << std::endl;
	}
	else{
	    std::cout<< "No mapping found for task id " << taskId << std::endl;
	    return -1;
	}
    }
    ...
    sstm << "SimulationState-" << taskId << ".dat";
    ...
    ofstream detailedOP;
    sstm << fname << "-DetailedOutput-" << taskId << ".dat";
